 ...
 * 
 */
public class SunMonPosition
{

	
	public final static double pi = Math.PI;
	public final static double DEG2RAD = pi/180.0;
	public final static double RAD2DEG = 180./pi;
	...
	public static double gMST2LMST(double gmst, double lon) {
		return(mod(gmst+RAD2DEG*lon/15, 24.));
	}
	...
	public static Coor ecl2Equ(Coor coor, double TDT) {
		...
		double eps = (23.+(26+21.45/60.)/60. + T*(-46.815 +T*(-0.0006 + T*0.00181) )/3600. )*DEG2RAD;
	...
	{		
		...
		double eg = 279.403303 * DEG2RAD;
		double wg = 282.768422 * DEG2RAD;
		double e = 0.016713;
		int a = 149598500; // km
		double diameter0 = 0.533128 * DEG2RAD; // angular diameter of Moon at a distance

		double MSun = 360 * DEG2RAD / 365.242191 * D + eg - wg;
		double nu = MSun + 360.0 * DEG2RAD / pi * e * Math.sin(MSun);
	...
	private static Coor moonPosition(Coor sunCoor, double TDT, Coor observer, double lmst, boolean useObs)
	{
		...
		double l0 = 318.351648 * DEG2RAD;
		double P0 = 36.340410 * DEG2RAD;
		double N0 = 318.510107 * DEG2RAD;
		double i = 5.145396 * DEG2RAD;
		double e = 0.054900;
		int a = 384401;                // km
		double diameter0 = 0.5181 * DEG2RAD;     // angular diameter of Moon at a distance
		double parallax0 = 0.9507 * DEG2RAD;     // parallax at distance a

		double l = 13.1763966 * DEG2RAD * D + l0;
		double MMoon = l - 0.1114041 * DEG2RAD * D - P0; // Moon's mean anomaly M
		double N = N0 - 0.0529539 * DEG2RAD * D;       // Moon's mean ascending node longitude
		double C = l - sunCoor.lon;
		double Ev = 1.2739 * DEG2RAD * Math.sin(2 * C - MMoon);
		double Ae = 0.1858 * DEG2RAD * Math.sin(sunCoor.anomalyMean);
		double A3 = 0.37 * DEG2RAD * Math.sin(sunCoor.anomalyMean);
		double MMoon2 = MMoon + Ev - Ae - A3;            // corrected Moon anomaly
		double Ec = 6.2886 * DEG2RAD * Math.sin(MMoon2);   // equation of centre
		double A4 = 0.214 * DEG2RAD * Math.sin(2 * MMoon2);
		double l2 = l + Ev + Ec - Ae + A4;                 // corrected Moon's longitude
		double V = 0.6583 * DEG2RAD * Math.sin(2 * (l2 - sunCoor.lon));
		...
		double mainPhase = 1.0 / 29.53 * 360 * DEG2RAD; // show 'Newmoon, 'Quarter' for +/-1 day arond the actual event
		double p = mod(moonCoor.moonAge, 90.0 * DEG2RAD);
		  
		  
		if (p < mainPhase || p > 90*DEG2RAD-mainPhase) p = 2*Math.round(moonCoor.moonAge / (90.*DEG2RAD));
	...
	private static Riseset gMSTRiseSet(Coor corr, double lon, double lat, double h)
	{
		...
		r1.rise = 24.0 + RAD2DEG / 15 * (-tagbogen + corr.ra - lon); // calculate GMST of rise of object
		r1.set = RAD2DEG / 15 * (+tagbogen + corr.ra - lon); // calculate GMST of set of object
	...
	private static Riseset riseSet(double jd0UT, Coor coor1, Coor coor2, double lon, double lat, double timeinterval, double altitude, boolean useAlt)
	{
		...
		if (!useAlt && altitude == 0.0) alt = 0.5 * coor1.diameter - coor1.parallax + 34.0 / 60 * DEG2RAD;
		...
		double T02 = T0 - lon * RAD2DEG / 15 * 1.002738; if (T02 < 0) T02 += 24;
	...
	public static Riseset sunRise(double JD, double deltaT, double lon, double lat, double zone, boolean recursive)
	{
		...
		{ // check and adjust to have rise/set time on local calendar day
			...
			risetemp = riseSet(jd0UT, coor1, coor2, lon, lat, 1, -6.*DEG2RAD);
			...
			risetemp = riseSet(jd0UT, coor1, coor2, lon, lat, 1, -12.*DEG2RAD);
			...
			risetemp = riseSet(jd0UT, coor1, coor2, lon, lat, 1, -18.*DEG2RAD);
